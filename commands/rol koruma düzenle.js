...
.setDescription(`<:krali:1160170285539008582>  ***Düzenlenen Rol Koruma Sistem Bilgilendirmesi***\n *Düzenlenen rolleri eski haline getirir.*\n`)
.addField(`<:krali:1160170285539008582>  **Nasıl Açacağım?**`,"`r!düzenlenen-rol-koruma aç`\n")
.addField(`<:krali:1160170285539008582>  **Nasıl Kapatacağım?**`,"`r!düzenlenen-rol-koruma kapat`")

if(!args[0]) return message.channel.send(emb)

const invex11 = new Discord.MessageEmbed()
.setColor('BLUE')
  .setDescription(`Düzenlenen Rol Koruma sistemi başarıyla açıldı.`)
...
.setColor('BLUE')
.setDescription(`Düzenlenen Rol Koruma sistemi kapatıldı.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
...
.setColor('BLUE')
.setDescription(`Düzenlenen Rol Koruma sistemi zaten kapalı durumda.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
...
.setColor('BLUE')
.setDescription(`Düzenlenen Rol Koruma sistemi zaten açık durumda.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()

if(args[0] === "aç"){
if(db.has(`duzrol.${message.guild.id}`)) return message.channel.send(inve0);
message.channel.send(invex11) 
db.set(`duzrol.${message.guild.id}`, "Açık")

}else if(args[0] === "kapat"){
if(!db.has(`duzrol.${message.guild.id}`)) return message.channel.send(inve1);
db.delete(`duzrol.${message.guild.id}`)
...
exports.conf = {
  enabled: true,
  guildOnly: true,
  aliases: ["düzenlenen-rol-koruma"],
  permLevel: 0
}

exports.help = {
  name: 'düzenlenen-rol-koruma'
};