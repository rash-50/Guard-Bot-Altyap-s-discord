...
.setDescription(`<:krali:1160170285539008582>  ***Bot İzin Sistem Bilgilendirmesi***\n *Sunucuya katılması için bota izin verir.*\n`)
.addField(`<:krali:1160170285539008582>  **Nasıl İzin Vereceğim?**`,"`r!bot-izin ID`\n")
.addField(`<:krali:1160170285539008582>  **Nasıl İzin Kaldıracağım?**`,"`r!bot-izin-kaldır ID`\n")

const invex1111 = new Discord.MessageEmbed()
.setColor('BLUE')
.setDescription(`${botid} ID'li botun izni devam ediyor.`)
...
.setColor('BLUE')
.setDescription(`**${botid}** ID'li bota **${message.guild.name}** sunucusunda **${message.author}** tarafından izin verildi.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
