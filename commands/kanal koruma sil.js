...
.setDescription(`<:krali:1160170285539008582>***Silinen Kanal Koruma Sistem Bilgilendirmesi***\n *Silinen kanalları yeniden oluşturur ve silenin tüm rollerini alır.*\n`)
.addField(`<:krali:1160170285539008582>  **Nasıl Açacağım?**`,"`r!silinen-kanal-koruma aç`\n")
.addField(`<:krali:1160170285539008582> > **Nasıl Kapatacağım?**`,"`r!silinen-kanal-koruma kapat`")

if(!args[0]) return message.channel.send(emb)

const invex11 = new Discord.MessageEmbed()
.setColor('BLUE')
  .setDescription(`Silinen Kanal Koruma sistemi başarıyla açıldı.`)
...
.setColor('BLUE')
.setDescription(`Silinen Kanal Koruma sistemi kapatıldı.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
...
.setColor('BLUE')
.setDescription(`Silinen Kanal Koruma sistemi zaten kapalı durumda.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
...
.setColor('BLUE')
.setDescription(`Silinen Kanal Koruma sistemi zaten açık durumda.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
...
if(args[0] === "aç"){
if(db.has(`silkanal.${message.guild.id}`)) return message.channel.send(inve0);
message.channel.send(invex11) 
db.set(`silkanal.${message.guild.id}`, "Açık")

}else if(args[0] === "kapat"){
if(!db.has(`silkanal.${message.guild.id}`)) return message.channel.send(inve1);
db.delete(`silkanal.${message.guild.id}`)
...
exports.conf = {
  enabled: true,
  guildOnly: true,
  aliases: ["silinen-kanal-koruma"],
  permLevel: 0
}

exports.help = {
  name: 'silinen-kanal-koruma'
};