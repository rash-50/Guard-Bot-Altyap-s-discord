...
.setDescription(`<:krali:1160170285539008582>  ***Eklenen Emoji Koruma Sistem Bilgilendirmesi***\n *Eklenen emojileri anında siler.*\n`)
.addField(`<:krali:1160170285539008582>  **Nasıl Açacağım?**`,"`r!eklenen-emoji-koruma aç`\n")
.addField(`<:krali:1160170285539008582>  **Nasıl Kapatacağım?**`,"`r!eklenne-emoji-koruma kapat`")

if(!args[0]) return message.channel.send(emb)

const invex11 = new Discord.MessageEmbed()
.setColor('BLUE')
  .setDescription(`Eklenen Emoji Koruma sistemi başarıyla açıldı.`)
...
.setColor('BLUE')
.setDescription(`Eklenen Emoji Koruma sistemi kapatıldı.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
...
.setColor('BLUE')
.setDescription(`Eklenen Emoji Koruma sistemi zaten kapalı durumda.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
...
.setColor('BLUE')
.setDescription(`Eklenen Emoji Koruma sistemi zaten açık durumda.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
if(args[0] === "aç"){
if(db.has(`acemo.${message.guild.id}`)) return message.channel.send(inve0);
message.channel.send(invex11) 
db.set(`acemo.${message.guild.id}`, "Açık")

}else if(args[0] === "kapat"){
if(!db.has(`acemo.${message.guild.id}`)) return message.channel.send(inve1);
db.delete(`acemo.${message.guild.id}`)
...
exports.conf = {
  enabled: true,
  guildOnly: true,
  aliases: ["açılan-emoji-koruma"],
  permLevel: 0
}

exports.help = {
  name: 'açılan-emoji-koruma'
};